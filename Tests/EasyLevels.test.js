...
describe("EasyLevels tests", () => {
  const navigation = {
    navigate: jest.fn(),
  };

  it("Renders the Easy Levels data", () => {
    const { getByText } = render(
      <CreditsContext.Provider value={mockCredits}>
        <EasyLevelsScreen navigation={navigation} levels={levelData} />
      </CreditsContext.Provider>
    );
    expect(getByText("Easy Levels")).toBeTruthy();
  });
