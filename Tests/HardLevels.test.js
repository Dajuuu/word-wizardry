...
describe("HardLevels tests", () => {
  const navigation = {
    navigate: jest.fn(),
  };

  it("Renders the Hard Levels data", () => {
    const { getByText } = render(
      <CreditsContext.Provider value={mockCredits}>
        <HardLevelsScreen navigation={navigation} levels={levelData} />
      </CreditsContext.Provider>
    );
    expect(getByText("Hard Levels")).toBeTruthy();
  });
